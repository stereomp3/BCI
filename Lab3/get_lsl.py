...
#     while True:
#         if eeg_buffer.shape[1] >= BUFFER_SIZE:
#             input_data = eeg_buffer[:, -BUFFER_SIZE:]  # shape: (4, 1249)
#             x_batch = torch.from_numpy(input_data).float().unsqueeze(0).unsqueeze(0).to(device)  # (1,1,4,1249)
#             with torch.no_grad():

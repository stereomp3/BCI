...
def main():
    ...
    for lbl in unique_labels:
        ...
        label_avr_eng.append((lbl, mean_energy_label, energy_mean_per_trial))
        # 6. 印出結果
        print(f"=== Label = {lbl} ===")
    ...
    label_avr_eng.sort(key=lambda x: x[1])
    for i in range(len(label_avr_eng)):
        average = label_avr_eng[i][1]  # average
        label = label_avr_eng[i][0]
        print(average)
        for t in label_avr_eng[i][2]:
            print(label_avr_eng[i][0], t)

    # for i in label_avr_eng:
    #     label_avr_eng +1

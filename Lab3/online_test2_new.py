...
classes = 2
model = EEGNet_SSVEP(4, 4999, classes).to(device)
checkpoint = torch.load("checkpoints/train-epoch498.pth", map_location=device)
model.load_state_dict(checkpoint['state_dict'])
model.eval()

# ======== Globals for Threading ============
BUFFER_SIZE = 4999  # samples needed per prediction
...
history = deque(maxlen=50)  # 保存最多 50 個 prediction
...
def report_history():
    while True:
        ...
        if history:
            ...
            print(f"[History Report] Most common prediction: {most_common}")
...
def read_eeg(inlet):
    ...
    while True:
        ...
        # sample = np.array(sample[0:2]).reshape(-1, 1)
...
def predict_loop():
    ...
    while True:
        ...
        x_batch = torch.from_numpy(input_data.copy()).float().unsqueeze(0).unsqueeze(0).to(device)
...
def ssvep_window():
    ...
    while True:
        ...
        phase = (t * frequencies[index]) % 1
        # === 開始呈現刺激 ===
        # for i in range(n_frames):
            # stimuli[index].opacity = 1.0 if pattern[i] else 0.0
            # stimuli[index].draw()
            # win.flip()
        stimuli[index].opacity = 1.0 if phase < 0.5 else 0.0
        stimuli[index].draw()
        win.flip()
...
def main():
    ...
    # 啟動背景執行的報告執行緒
    threading.Thread(target=report_history, daemon=True).start()
